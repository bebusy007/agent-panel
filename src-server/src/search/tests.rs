...
mod tests {
    ...
    fn test_snippet_contains_highlight_mark() {
        ...
        create_test_session(
            ...
            &[sample_user_message(
                "u1",
                "s1",
                "This is a test with keyword inside",
                "2026-05-01T10:00:00Z",
            )],
        );
    ...
    fn test_results_sorted_by_timestamp_desc() {
        ...
        // Most recent first
        assert!(
            result.hits[0].timestamp.as_deref().unwrap()
                > result.hits[1].timestamp.as_deref().unwrap()
        );
        assert!(
            result.hits[1].timestamp.as_deref().unwrap()
                > result.hits[2].timestamp.as_deref().unwrap()
        );
    }
