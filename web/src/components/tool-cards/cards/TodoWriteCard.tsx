...
export function TodoWriteCard({ tool }: { tool: Message; result?: Message }) {
  ...
  const list = (Array.isArray(input.todos) ? input.todos : Array.isArray(input.newTodos) ? input.newTodos : []) as TodoItem[];

  if (list.length === 0) {
    return (
      <p className="px-3 pb-3 text-[11px] italic text-muted-foreground">
        无 todo 项
      </p>
    );
  }
      ...
      {list.map((t, i) => {
        ...
        return (
          ...
          >
            ...
            />
            <span className="flex-1">{text || <span className="italic text-muted-foreground">(空)</span>}</span>
          </li>
        );
      })}
    </ul>
