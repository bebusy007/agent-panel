    ...
    <div className="space-y-1 px-3 pb-3">
      <div className="flex items-center justify-between rounded-md border border-border bg-secondary/50 px-2.5 py-1 text-[11px]">
        ...
        <div className="flex items-center gap-2 text-muted-foreground">
          ...
          {hunks.length > 0 && <CopyButton text={hunksToString(hunks)} label="复制 diff" />}
        </div>
      </div>
      {hunks.length === 0 ? (
        <p className="text-[11px] italic text-muted-foreground">
          无可解析的 diff（输入缺 old_string/new_string）
        </p>
      ) : (
      ...
      {tail && (
        <details className="rounded-md border border-border bg-background/40">
          ...
          <pre className="max-h-[200px] overflow-auto whitespace-pre-wrap break-words px-2.5 pb-2 font-mono text-[11px] text-muted-foreground">
            {tail}
          </pre>
        </details>
      )}
...
function extractTailText(result?: Message): string {
  ...
  return result.toolOutput.length > EDIT_TAIL_MAX_LEN
    ? result.toolOutput.slice(0, EDIT_TAIL_MAX_LEN) + '…'
    : result.toolOutput;
}
