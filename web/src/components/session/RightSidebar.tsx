...
export function RightSidebar({ summary, messages, turns, activeTurnIndex, onScrollToMessage }: Props) {
  ...
  const tabs: Array<{ key: TabKey; icon: React.ComponentType<{ className?: string }>; label: string; count?: number }> = [
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      {tab === "files" && (
        <FilesPanel entries={fileEntries} turns={turns} activeTurnIndex={activeTurnIndex} onScrollToMessage={onScrollToMessage} />
      )}
      {tab === "info" && <InfoPanel summary={summary} messages={messages} fileEntries={fileEntries} toolUses={toolUses} />}
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      <div className="flex-1 overflow-y-auto py-0.5">
        {filteredToolUses.map((m) => {
          ...
          const status: "success" | "error" | "pending" = result?.toolStatus === "error"
            ? "error"
            : result
              ? "success"
              : "pending";
          ...
          return (
            ...
            >
              ...
              />
              <span className="shrink-0 max-w-[100px] truncate text-[11px] font-medium text-muted-foreground">{display}</span>
              {detail && (
        ...
        return (
          ...
          >
            ...
            >
              ...
              <span className="shrink-0 text-[10px] tabular-nums text-muted-foreground">{files.length}</span>
            </button>
            {files.map((e, i) => (
              <button
                key={`${e.path}-${i}`}
...
function InfoPanel({
  ...
  if (!summary) {
    return <div className="flex flex-1 items-center justify-center text-xs text-muted-foreground">无会话信息</div>;
  }
  return (
    <div className="flex-1 overflow-y-auto p-3 space-y-3 text-xs">
      <Section title="会话">
        ...
        {summary.sessionIdRaw && <Row label="ID" mono>{summary.sessionIdRaw.slice(0, SESSION_ID_DISPLAY_LEN)}…</Row>}
        {summary.model && <Row label="模型" mono>{summary.model}</Row>}
        <Row label="状态">
          {false ? (
      ...
      <Section title="环境">
        {summary.cwd && <Row label="cwd" mono mono-wrap>{summary.cwd?.replace(/^\/Users\/[^\/]+/, "~") || summary.cwd}</Row>}
        {summary.gitBranch && <Row label="分支" mono>{summary.gitBranch}</Row>}
      </Section>
      ...
      <Section title="时间">
        {summary.startedAt && <Row label="开始">{new Date(summary.startedAt).toLocaleString()}</Row>}
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      ...
      <div className="space-y-1 rounded-md border border-border bg-background/40 px-2.5 py-1.5">{children}</div>
    </div>
  );
}
...
function Row({
  ...
  ...rest
}: { label: string; mono?: boolean; "mono-wrap"?: boolean; children: React.ReactNode }) {
...
function ContextPanel({ summary, messages }: { summary: SessionSummary | undefined; messages: Message[] }) {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-3 space-y-3 text-xs">
      <Section title="累计 token">
        <Row label="总计">
          <span className="tabular-nums">
            {formatTokens(summary?.tokensTotal ?? 0)}
          </span>
        </Row>
      </Section>
      {points.length > 0 && (
...
function describeToolDetail(input: unknown): string {
  ...
  for (const k of ["file_path", "path", "command", "cmd", "pattern", "query", "url", "description", "prompt"]) {
    const v = o[k];
    if (typeof v === "string" && v.trim()) return v.length > RIGHT_SIDEBAR_VALUE_MAX_LEN ? v.slice(0, RIGHT_SIDEBAR_VALUE_MAX_LEN) + "…" : v;
  }
  return "";
}
