  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-1.5">
        ...
        {xmlDisplay && XmlIcon && xmlConfig?.showHeaderBadge !== false && (() => {
      ...
      {(m.images?.length ?? 0) > 0 && (
        <ImageThumbnailStrip sessionId={sessionId} message={m} />
      )}
      {!showBody && (
          ...
          {highlight
            ? highlightJsx(
                m.text ||
                  m.toolOutput ||
                  (m.toolInput
                    ? JSON.stringify(m.toolInput).slice(0, TOOL_INPUT_PREVIEW_LEN)
                    : ""),
                highlight,
              )
            : m.text ||
