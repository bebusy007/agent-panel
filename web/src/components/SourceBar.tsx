...
export function SourceBar({ items, max }: { items: Array<{ label: string; type: string; count: number }>; max?: number }) {
    ...
    <div className="space-y-2.5">
      {top.map((it) => (
        <div key={it.type + ":" + it.label} className="flex items-center gap-3">
          ...
          <div className="w-10 text-right text-sm tabular-nums text-muted-foreground">{it.count}</div>
        </div>
      ))}
    </div>
  );
