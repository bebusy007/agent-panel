  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-5 pt-3 pb-2 border-b border-border flex flex-wrap items-center gap-2">
        <span className="text-xs rounded bg-secondary px-1.5 py-0.5 text-muted-foreground">{mcp.source}</span>
        {mcp.configuredIn.filter(s => s !== mcp.source).map(s => (
          <span key={s} className="text-xs rounded bg-secondary px-1.5 py-0.5 text-muted-foreground">{s}</span>
        ))}
        ...
        {mcp.description && (
          <section className="rounded-lg border border-border bg-background/40 p-3 text-xs">
            ...
            <div className="flex items-start gap-2">
              ...
              <code className="font-mono break-all text-muted-foreground text-[11px] flex-1">{mcp.description}</code>
              <button onClick={() => copy(mcp.description!)} className="text-muted-foreground hover:text-foreground transition-colors shrink-0">
                {copied === mcp.description ? <Check className="size-3 text-emerald-400" /> : <Copy className="size-3" />}
              </button>
            </div>
          </section>
        )}
          ...
          <section className="rounded-lg border border-border bg-background/40 p-3 text-xs">
            ...
            <ul className="space-y-1">
              {mcp.configuredIn.map(src => (
                <li key={src} className="font-mono text-[11px] text-muted-foreground">{src}</li>
              ))}
            </ul>
          </section>
        )}
          ...
          <section>
            ...
            <ul className="space-y-1.5">
              {mcp.toolNames.map((name) => (
                <li key={name} className="rounded-lg border border-border bg-background/30 px-3 py-2">
