...
function sortValue(s: RustSessionSummary, key: SortKey): number | string | undefined {
  switch (key) {
    case "lastActivity": return s.lastActivity || "";
    case "startedAt": return s.startedAt || s.lastActivity || "";
    case "messageCount": return s.messageCount ?? 0;
    case "tokensTotal": return s.tokensTotal;
  }
}

function compareSessions(a: RustSessionSummary, b: RustSessionSummary, key: SortKey, dir: SortDir): number {
...
export default function SessionsView() {
  ...
  const setSort = (key: SortKey, dir: SortDir) => {
    ...
    if (key === DEFAULT_SORT.key && dir === DEFAULT_SORT.dir) {
      ...
      next.set("sort", key); next.set("dir", dir);
    }
  ...
  const refreshAll = () => {
    ...
    refetch();
    projects.refetch();
    api.trashList().then((d) => setTrashCount(d.total)).catch(() => {});
  ...
  const doSearch = useCallback(async (q: string, msgType: MessageTypeFilter, project: string, offset = 0, append = false) => {
    if (q.trim().length < 2) {
      if (!append) { setMessageHits([]); setMessageTotalMatches(0); }
      return;
    }
    ...
    try {
      ...
      const r = await api.searchMessages({ query: q, filters: filters as any, limit: PAGE_SIZE, offset });
      if (append) {
  ...
  const handleAction = async (s: RustSessionSummary, action: "hide" | "show" | "trash" | "restore" | "permanent") => {
    if (action === "trash") {
      setConfirm({
        title: "移到回收站",
        description: <>将「<span className="text-foreground">{s.title || s.id}</span>」移到回收站。</>,
        tone: "danger",
        onConfirm: async () => {
    ...
    if (action === "permanent") {
      setConfirm({
        title: "永久删除",
        description: <>将永久删除「<span className="text-foreground">{s.title || s.id}</span>」，无法恢复。</>,
        requireType: "删除",
        tone: "danger",
        onConfirm: async () => {
  ...
  const handleBatchTrash = async () => {
    ...
    setConfirm({
      ...
      onConfirm: async () => {
        ...
        flash(`已处理 ${r.trashed.length}/${filePaths.length}` + (r.errors.length ? `，失败 ${r.errors.length}` : ""));
    ...
    <div className="space-y-5 animate-fade-in">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {!isSearchActive && (
            <div className="inline-flex items-stretch rounded-md border border-border overflow-hidden">
              <span className="text-[11px] text-muted-foreground px-2 py-1.5 bg-background/50 border-r border-border self-center">排序</span>
              <select
                value={sortKey}
          ...
          >
            ...
            回收站{trashCount > 0 && <span className="ml-0.5 text-[10px] bg-amber-500/30 text-amber-200 px-1.5 rounded">{trashCount}</span>}
          </Link>
          <button
            onClick={() => refreshAll()}
            className="text-xs px-3 py-1.5 rounded-md border border-border text-muted-foreground hover:text-foreground hover:border-border transition-colors inline-flex items-center gap-1.5"
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {!messageSearchLoading && messageTotalMatches > 0 && (
              <span className="text-[11px] text-muted-foreground ml-auto">
                匹配 <span className="text-foreground tabular-nums">{messageTotalMatches}</span> 条消息，{messageSearchTime}ms
              </span>
            )}
      ...
      {!isSearchActive ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-xs text-muted-foreground/60 mt-2">支持搜索用户消息、助手回复、工具调用等全部内容</p>
        </div>
      ) : (
        ...
        <div className="space-y-4">
            ...
            <section>
              ...
              <div className="space-y-1.5">
                {titleMatches.slice(0, 20).map((s) => (
                  ...
                  >
                    <span className="rounded bg-blue-500/10 px-1.5 py-0.5 text-[10px] font-medium text-blue-400">标题</span>
                    <span className={cn("shrink-0 rounded-full px-1.5 py-0.5 text-[10px] ring-1 ring-inset", sourceColor(s.source))}>
                    ...
                    {s.lastActivity && (
                      <span className="shrink-0 text-[11px] text-muted-foreground">{formatRelative(s.lastActivity)}</span>
                    )}
                  </button>
                ))}
          ...
          <section>
            ...
            {messageHits.length > 0 && (
              <ul className={cn("space-y-1.5 transition-opacity", messageSearchLoading && "opacity-60")}>
                {messageHits.map((h, i) => (
                  <li key={`${h.sessionId}-${h.messageId}-${i}`}>
                    <button
                      onClick={() => openOverlay(`/sessions/${encodeURIComponent(h.sessionId)}?msg=${encodeURIComponent(h.messageId)}`)}
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className={cn(
                            ...
                            {h.role === "user" ? <User className="size-2.5" /> : <Bot className="size-2.5" />}
                            {h.role === "user" ? "用户" : "助手"}
                          </span>
                          {h.projectName && (
                            <span className="truncate font-mono text-[10px] text-muted-foreground max-w-[200px]">
                              {h.projectName.replace(/^-/, "").replace(/-/g, "/").replace(/^Users\/[^/]+\//, "~/")}
                            </span>
                          )}
                          {h.timestamp && (
              ...
              <div className="pt-3 text-center">
                ...
                >
                  {loadingMore ? (
                    ...
                    <><ChevronDown className="size-3" /> 加载更多（已显示 {messageHits.length}/{messageTotalMatches}）</>
                  )}
                </button>
              </div>
            )}
      ...
      {confirm && (
        <ConfirmDialog
          ...
          onConfirm={async (cascadeIds) => { await confirm.onConfirm(cascadeIds); setConfirm(null); }}
          title={confirm.title}
