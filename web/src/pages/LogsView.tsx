...
export default function LogsView() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setHourFilter("all");
    api.logsContent(selectedFile).then((res) => {
  ...
  const hourOptions = useMemo(() => allHourGroups.map((hg) => ({ key: hg.hourKey, label: hg.hourLabel, count: hg.count })), [allHourGroups]);
  const visibleGroups = useMemo(() => {
    if (hourFilter === "all") return allHourGroups;
    return allHourGroups.filter((hg) => hg.hourKey === hourFilter);
  }, [allHourGroups, hourFilter]);
  const visibleCount = useMemo(() => visibleGroups.reduce((s, g) => s + g.count, 0), [visibleGroups]);
...
function LogLine({ entry }: { entry: LogEntry }) {
  ...
  return (
    <div className="flex items-start gap-2 px-4 py-1 hover:bg-secondary/40 transition-colors border-b border-border/30 leading-5">
      ...
      <span className={cn("w-12 shrink-0 text-right font-semibold", levelCls)}>
        {entry.level}
      </span>
...
function toLocalHourSecond(ts: string): { hourKey: string; hourLabel: string; second: string } {
  ...
  try {
    const d = new Date(ts);
    if (isNaN(d.getTime())) return { hourKey: ts.slice(0, 13), hourLabel: ts.slice(0, 13), second: ts.slice(0, 19) };
