...
import { Sparkles, Wrench, Webhook, Bot, Search, X, Terminal, Package } from 'lucide-react';
import {
  api,
  type RustHookEntry,
  type RustAgentEntry,
  type RustCommandEntry,
  type RustPluginEntry,
} from '@/lib/api';
  ...
  return (
    <div className="space-y-4">
      <p className="text-xs text-muted-foreground">
        共 <span className="text-foreground tabular-nums">{hooks.length}</span> 个 hook， 分布在{' '}
        <span className="text-foreground tabular-nums">{grouped.length}</span> 个事件类型。
      </p>
              ...
              <div key={`${event}-${i}`} className="rounded-lg border border-border bg-card p-3">
                ...
                <div className="mt-2 space-y-1">
                  {h.commands.map((cmd, ci) => (
                    <pre
                      key={ci}
                      className="overflow-x-auto rounded-md border border-border bg-background/60 px-2.5 py-1.5 font-mono text-xs text-emerald-200"
                    >
                      {cmd}
                    </pre>
                  ))}
    ...
    <div className="space-y-3">
      <div className="relative max-w-md">
        ...
        <input
          ...
          placeholder="筛选 agent 名称 / 描述（≥2字符）"
      ...
      <ul className="grid grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-3">
        {filtered.map((a) => (
          <li key={a.name} className="rounded-lg border border-border bg-card p-3">
            ...
            {a.description && (
              <p className="mt-1.5 line-clamp-3 text-xs text-muted-foreground">{a.description}</p>
            )}
...
function CommandsSection() {
  ...
  const filtered = useMemo(() => {
    ...
    return commands.filter(
      (c) => c.command.toLowerCase().includes(q) || c.skill.toLowerCase().includes(q),
    );
  }, [commands, debounced]);
      ...
      <div className="flex items-center gap-2">
        <div className="relative max-w-md flex-1">
          ...
          <input
            ...
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder="筛选命令名 / 所属 skill（≥2字符）"
            className="h-9 w-full rounded-lg border border-border bg-background/40 pl-9 pr-9 text-sm focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary/40"
          />
          {query && (
            <button
        ...
        <span className="text-xs text-muted-foreground">
          共 <span className="text-foreground tabular-nums">{filtered.length}</span> 个
        </span>
      </div>
      <ul className="grid grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-3">
        {filtered.map((c, i) => (
          <li key={`${c.command}-${i}`} className="rounded-lg border border-border bg-card p-3">
            <div className="flex items-center gap-2">
              ...
              <code className="truncate text-sm font-mono font-semibold text-blue-300">
                {c.command}
              </code>
            </div>
...
function PluginsSection() {
  ...
  const filtered = useMemo(() => {
    ...
    return plugins.filter(
      (p) => p.name.toLowerCase().includes(q) || p.scope.toLowerCase().includes(q),
    );
  }, [plugins, debounced]);
        ...
        {filtered.map((p, i) => (
          <li key={`${p.name}-${i}`} className="rounded-lg border border-border bg-card p-3">
            <div className="flex items-center gap-2">
              ...
              {p.version && (
                <code className="rounded bg-secondary px-1.5 py-px text-[10px] font-mono text-muted-foreground">
                  {p.version}
                </code>
              )}
            </div>
