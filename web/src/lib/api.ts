...
export const rustApi = {
  ...
  statsActivity: (weeks = 52) =>
    request<{
      weeks: number;
      days: Array<{ date: string; tokens: number; sessions: number; messages: number }>;
  ...
  skill: (id: string) =>
    request<{ skill: RustSkillSummary } | { error: string }>(
  ...
  mcp: (name: string) =>
    request<{ mcp: RustMcpSummary } | { error: string }>(`/api/mcps/${encodeURIComponent(name)}`),
  ...
  sessionDetail: (id: string) => request<SessionFull>(`/api/sessions/${encodeURIComponent(id)}`),
  sessionSearch: (id: string, q: string, limit = 50) =>
    request<{
      q: string;
      hits: Array<{ messageId: string; role: string; snippet: string }>;
      total: number;
    }>(`/api/sessions/${encodeURIComponent(id)}/search?q=${encodeURIComponent(q)}&limit=${limit}`),
  sessionsProjects: () => request<{ projects: RustProjectEntry[] }>('/api/sessions/projects'),
  ...
  sessionSubagent: (sessionId: string, agentHash: string) =>
    request<{
      meta: { agentHash: string; messageCount: number; filePath: string };
  ...
  commands: () =>
    request<{ commands: RustCommandEntry[]; total: number }>('/api/extensions/commands'),
  extensionsSummary: () =>
    request<{ hooks: number; agents: number; plugins: number }>('/api/extensions/summary'),
