...
describe('highlightHtml', () => {
  ...
  it('is case-insensitive', () => {
    expect(highlightHtml('ABC abc', 'abc')).toBe('<mark>ABC</mark> <mark>abc</mark>');
  });
  ...
  it('escapes HTML in matched text', () => {
    expect(highlightHtml('<script>', 'script')).toBe('&lt;<mark>script</mark>&gt;');
  });

  it('handles multiple adjacent matches', () => {
    expect(highlightHtml('aaa', 'a')).toBe('<mark>a</mark><mark>a</mark><mark>a</mark>');
  });
});
