...
describe('cleanupPromptPreview', () => {
  ...
  it('strips <user_query> wrapper', () => {
    expect(cleanupPromptPreview('<user_query>Hello world</user_query>')).toBe('Hello world');
  });

  it('strips <user_message> wrapper', () => {
    expect(cleanupPromptPreview('<user_message>Test prompt</user_message>')).toBe('Test prompt');
  });
  ...
  it('collapses slash command without args', () => {
    const raw = '<command-message>help</command-message><command-name>/help</command-name>';
    expect(cleanupPromptPreview(raw)).toBe('/help');
  });

  it('strips local-command envelopes', () => {
    expect(cleanupPromptPreview('<local-command-stdout>output text</local-command-stdout>')).toBe(
      'output text',
    );
  });
  ...
  it('collapses whitespace', () => {
    expect(cleanupPromptPreview('  lots   of    spaces  ')).toBe('lots of spaces');
  });
