...
describe('useAsync', () => {
  ...
  it('handles rejection', async () => {
    const { result } = renderHook(() => useAsync(() => Promise.reject(new Error('fail'))));
    await waitFor(() => expect(result.current.loading).toBe(false));
  ...
  it('refetch triggers re-fetch', async () => {
    let count = 0;
    const { result } = renderHook(() => useAsync(() => Promise.resolve(++count)));
...
describe('useCachedAsync', () => {
  ...
  it('returns cached data immediately on re-mount', async () => {
    ...
    const { result: result2 } = renderHook(() => useCachedAsync('key2', fn));
...
describe('useDebounced', () => {
  ...
  it('debounces value updates', () => {
    const { result, rerender } = renderHook(({ val }) => useDebounced(val, 100), {
      initialProps: { val: 'a' },
    });
    rerender({ val: 'b' });
