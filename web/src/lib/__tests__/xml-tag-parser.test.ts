...
describe("detectXmlTag", () => {
  ...
  it("detects registered tags", () => {
    expect(detectXmlTag("<task-notification>x</task-notification>")).toBe(
      "task-notification",
    );
    expect(detectXmlTag("<system-reminder>x</system-reminder>")).toBe(
      "system-reminder",
    );
    expect(
      detectXmlTag('<teammate-message teammate_id="a">x</teammate-message>'),
    ).toBe("teammate-message");
  });
...
describe("parseXmlTag", () => {
  ...
  it("parses task-notification with nested sections", () => {
    ...
    expect(result!.sections.find((s) => s.tag === "task-id")?.content).toBe(
      "42",
    );
  });
